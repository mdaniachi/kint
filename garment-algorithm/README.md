...
- **Effect panel**: Density, Character size, **Mask edge**, Edge influence, **Flow**, **Size by light**, Randomness, Original garment, Algorithm opacity, Ink direction, Colour, and the character set. Parameters are reinterpreted per effect (e.g. in Contour Lines, Density is the number of bands and Character size is line weight + smoothing).
- **Character set presets**: click the set to open the list — seven built-ins (ASCII, Blocks, Density, Long ramp, Dots, Bars, Binary), then anything saved. Type in the field below to build one from scratch, and **+ Save current as preset** names and keeps it. Saved presets live in `localStorage`: they survive reloads but stay on that browser, which is the honest fit for a tool with no backend.
- The grid step is measured from **the widest glyph actually in use**, not from a fixed `#`. A character the system font lacks falls back to another face with its own advance width, and a grid stepped by the wrong width would overlap or gap.
